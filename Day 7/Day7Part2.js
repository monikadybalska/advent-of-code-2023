...
readFileAsync("Day7").then(function (data) {
    ...
    for (let i = 0; i < set.length; i++) {
        ...
        let cardValues = Object.values(hand["cards"])
        let max = Math.max(...cardValues)
        let maxValues = Object.keys(hand["cards"]).filter((el) => hand["cards"][el] === max)
        for (let j = 0; j < handName.length; j++) {
            if (handName[j] === "J") {
                hand["cards"]["J"] -= 1
                if (hand["cards"][maxValues[0]] !== 5) {
                    hand["cards"][maxValues[0]] += 1
                }
            }
        }
        let values = Object.values(hand["cards"]).filter((el) => el !== 0).sort(compareFn)
